...
class NASNetAMobile(nn.Module):
    ...
    def __init__(self, num_classes=1000, stem_filters=32, penultimate_filters=1056, filters_multiplier=2, stem_stride=2, **kwargs):
        ...
        self.conv0.add_module('conv', nn.Conv2d(in_channels=3, out_channels=self.stem_filters, kernel_size=3, padding=0, stride=stem_stride,
                                                bias=False))
...
@register_model
def nasnetamobile(num_classes=1000, pretrained=False, **kwargs):
...
def better_nasnetamobile(num_classes=1000, pretrained=False, **kwargse):
    ...
    if pretrained:
       ...
       #     model.last_linear = new_last_linear
        ...
        settings = pretrained_settings['nasnetamobile']['imagenet']
        model = NASNetAMobile(num_classes=num_classes, stem_stride=1)
        model.input_space = settings['input_space']
        model.input_size = settings['input_size']
        model.input_range = settings['input_range']
