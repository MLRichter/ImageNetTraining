...
if __name__=='__main__':
    ...
    import torch
    model = nfnet_f0() # replace with any torch module-object-returning function
    model.eval()
    graph = create_graph_from_pytorch_model(model, input_res=(1, 3, 192, 192))
    flops = FlopCountAnalysis(model, torch.ones(1, 3, 192, 192)).total()
    imin, _ = input_resolution_range(graph)
    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print('Imin:', imin, '\tGFLOPS:', flops / 1000000000, "MParams:", n_parameters / 1000000)
    visualize_architecture(graph, "NFNET").view()